...
import type { AdminSmsRecordDto, DailySmsJobRunDto } from 'shared/contracts';
...
/** "Daily Scheme SMS" operational panel (spec 3.31/3.32) — a manual trigger + recent-run table,
 * not a full analytics dashboard. Polls while the latest run is still 'running' so the admin sees
 * it complete without needing to refresh. */
const DailySmsJobPanel: React.FC<{ theme: ThemeColors }> = ({ theme }) => {
  const [runs, setRuns] = useState<readonly DailySmsJobRunDto[]>([]);
  ...
  useEffect(() => {
    if (hasRunningRun && !pollRef.current) {
      pollRef.current = setInterval(load, 4000);
    } else if (!hasRunningRun && pollRef.current) {
  ...
  const handleRunNow = async () => {
    setTriggering(true);
    try {
      await api.admin.runDailySmsJob();
      load();
    } catch {
      // no-op — the runs table/error state below reflects reality on next load
    } finally {
      setTriggering(false);
    }
  };
  ...
  return (
    <div style={{ backgroundColor: theme.surface, borderRadius: '20px', border: `1.5px solid ${theme.border}`, padding: '20px', marginBottom: '24px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', marginBottom: '14px' }}>
        <div>
          <h3 style={{ fontSize: '18px', fontWeight: '800', color: theme.textHeading, margin: '0 0 4px 0' }}>Daily Scheme SMS</h3>
          <p style={{ fontSize: '12.5px', color: theme.textMuted, margin: 0 }}>
            Automated daily "best scheme for you today" SMS. Manual runs still respect the one-per-user-per-day guarantee.
          </p>
        </div>
        <button
          ...
          style={{
            ...
            fontWeight: 800,
            fontSize: '13px',
            cursor: triggering || hasRunningRun ? 'default' : 'pointer',
            opacity: triggering || hasRunningRun ? 0.6 : 1,
          }}
        >
          {hasRunningRun ? 'Job running…' : triggering ? 'Starting…' : 'Run daily job now'}
        </button>
      </div>

      {status === 'loading' && <div style={{ color: theme.textMuted, fontSize: '13px' }}>Loading job history…</div>}
      {status === 'error' && <div style={{ color: theme.alertUrgent, fontSize: '13px' }}>Could not load job history right now.</div>}
        ...
        <>
          {latest && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '10px', marginBottom: '16px' }}>
              {[
                { label: 'Last run', value: new Date(latest.startedAt).toLocaleString() },
                { label: 'Status', value: latest.status },
                { label: 'Considered', value: String(latest.usersConsidered) },
                { label: 'Sent / Enqueued', value: String(latest.messagesEnqueued) },
                { label: 'Skipped', value: String(latest.usersSkipped) },
              ].map((stat) => (
                <div key={stat.label} style={{ backgroundColor: theme.surfaceSubtle, borderRadius: '12px', padding: '10px 12px' }}>
                  <div style={{ fontSize: '11px', color: theme.textMuted, fontWeight: 700 }}>{stat.label}</div>
                  <div style={{ fontSize: '14px', color: theme.textHeading, fontWeight: 800 }}>{stat.value}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {runs.length === 0 ? (
            <div style={{ color: theme.textMuted, fontSize: '13px', padding: '8px 0' }}>No daily SMS job has run yet.</div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12.5px' }}>
                <thead>
                  <tr style={{ color: theme.textMuted, textAlign: 'left' }}>
                    ...
                    <th style={{ padding: '6px 10px' }}>Considered</th>
                    <th style={{ padding: '6px 10px' }}>Enqueued</th>
                    <th style={{ padding: '6px 10px' }}>Skipped</th>
                    <th style={{ padding: '6px 10px' }}>No suitable scheme</th>
                  </tr>
                </thead>
                <tbody>
                  {runs.map((run) => (
                    <tr key={run.id} style={{ borderTop: `1px solid ${theme.border}`, color: theme.textBody }}>
                      <td style={{ padding: '8px 10px' }}>{new Date(run.startedAt).toLocaleString()}</td>
                      <td style={{ padding: '8px 10px' }}>{run.triggerSource === 'admin_manual' ? 'Manual' : 'Scheduler'}</td>
                      <td style={{ padding: '8px 10px' }}>
                        <span
                          style={{
                            backgroundColor: run.status === 'completed' ? '#dcfce7' : run.status === 'failed' ? '#fee2e2' : '#fef3c7',
                            color: run.status === 'completed' ? '#166534' : run.status === 'failed' ? '#991b1b' : '#92400e',
                            padding: '3px 8px',
                            borderRadius: '8px',
                            fontWeight: 800,
                          }}
                        ...
                        {run.errorSummary && <div style={{ fontSize: '11px', color: theme.alertUrgent, marginTop: '4px' }}>{run.errorSummary}</div>}
                      </td>
                      <td style={{ padding: '8px 10px' }}>{run.usersConsidered}</td>
                      <td style={{ padding: '8px 10px' }}>{run.messagesEnqueued}</td>
                      <td style={{ padding: '8px 10px' }}>{run.usersSkipped}</td>
                      <td style={{ padding: '8px 10px' }}>{run.noSuitableSchemeCount}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
